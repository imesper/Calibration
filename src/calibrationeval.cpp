...
CalibrationEval::CalibrationEval(QString directory)
    : rmse(0.0)
    , mea(0.0)
    , smea(0.0)
    , c_rmse(0.0)
...
CalibrationEval::CalcError()
{
    ...
    for (auto& pset : m_pointsets) {
        ...
        for (int i = 0; i < pset.bfs.size(); ++i) {
            pcl::PointCloud<pcl::PointXYZRGBL>::Ptr points_pc(new pcl::PointCloud<pcl::PointXYZRGBL>());
...
//            viewer.addPointCloud<pcl::PointXYZRGBL>(points_pc,
//                                                    "cloud " + std::to_string(i));
//            viewer.addPointCloud<pcl::PointXYZRGB>(cloud,
//                                                   "F cloud " + std::to_string(i));
        ...
        for (int i = 0; i < pset.bfs.size(); ++i) {
            ...
            *cloud = *pset.bfs[i].cloud();
            ...
            if( i == 0){
                pcl::transformPointCloud(
                            *(cloud), *(cloud), pset.transformations[i] );
                pcl::transformPointCloud(
                            *(points_pc), *(points_pc), pset.transformations[i] );
            }
             if(i == 1 ) {
                 pcl::transformPointCloud(
                             *(cloud), *(cloud), pset.transformations[0] * pset.bfs[0].charucoTransformation() *
                         pset.bfs[i].charucoTransformation().inverse());
                 pcl::transformPointCloud(
                             *(points_pc), *(points_pc), pset.transformations[0] *pset.bfs[0].charucoTransformation() *
                         pset.bfs[i].charucoTransformation().inverse());
             }
             if(i == 2 || i == 3 ) {
                 Eigen::Affine3d A_C =
             ...
             if(i == 4 || i == 5 ) {
                 ...
                 pcl::transformPointCloud(
                             *(cloud), *(cloud), B_C);
                 pcl::transformPointCloud(
                             *(points_pc), *(points_pc), B_C );
             }

//            viewer.addPointCloud<pcl::PointXYZRGBL>(points_pc,
//                                                    "c cloud " + std::to_string(i));

//            viewer.addPointCloud<pcl::PointXYZRGB>(cloud,
//                                                   "Fc cloud " + std::to_string(i));
            ...
            for (size_t k = 0; k < points_pc->points.size(); k++) {
                ...
                if (it != pset.m_pointsPRef[idx]->points.end()) {
                    ...
                    if (!dis)
                        continue;
//                    viewer.addArrow(p1, p2, 255, 0, 0, false, std::to_string(p1.label +
//                                      rand()));
                    c_meas_count++;
                    pset.c_mea += dis;
...
//        viewer.spin();
        ...
        std::cout << "Charuco RMSE: " << pset.c_rmse << " for " << c_meas_count << " Points."
                  << endl;
        rmse += pset.rmse;
        mea += pset.mea;
        smea += pset.smea;
    ...
    std::cout << "Total charuco RMSE: " << c_rmse << " for " << c_meas_count << " Points." << endl;
}
